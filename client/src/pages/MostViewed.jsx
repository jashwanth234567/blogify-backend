...
const MostViewed = () => {
  ...
  const fetchMostViewed = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get("/api/posts/most-viewed", {
      ...
      toast.error("Failed to load top viewed posts.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex flex-col items-center text-center mb-10">
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold text-slate-900 dark:text-white mb-2">Most Viewed</h1>
          <p className="text-slate-500 dark:text-slate-450 text-sm max-w-md">Top articles with the highest amount of reader traffic and views.</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-slate-400">No viewed posts found.</div>
        ) : (
            ...
            {blogs.map(blog => (
              ...
              >
                ...
                <div className="border-t border-slate-100 dark:border-slate-800 pt-3 mt-4 flex items-center justify-between text-xs text-slate-400 dark:text-slate-500">
                  ...
                  <div className="flex items-center gap-2 font-bold text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-950/20 px-2.5 py-1 rounded-xl">
